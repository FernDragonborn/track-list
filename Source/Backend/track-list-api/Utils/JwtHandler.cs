...
internal static class JwtHandler
{
	...
	private const string TokenVersionStr = "tokenVersion";
	...
	public static Result<(Guid UserId, int TokenVersion)> ValidateRefreshTokenAndGetSession(string refreshToken)
	{
		var tokenHandler = new JwtSecurityTokenHandler();
		string? userId;
		string? tokenVersion;
		...
		try
		{
			if (string.IsNullOrWhiteSpace(refreshToken))
				return Result.Fail<(Guid UserId, int TokenVersion)>($"{RefreshStr} token, was null or empty.");
			...
			if (claims.Claims.FirstOrDefault(c => c.Type == TokenTypeStr)?.Value != RefreshStr)
				return Result.Fail<(Guid UserId, int TokenVersion)>($"{RefreshStr} token type is not \'{RefreshStr}\'.");

			userId = claims.Claims.FirstOrDefault(c => c.Type == "id")?.Value;
			if (string.IsNullOrWhiteSpace(userId))
				return Result.Fail<(Guid UserId, int TokenVersion)>($"{RefreshStr} token \'id\' claim was empty.");
			tokenVersion = claims.Claims.FirstOrDefault(c => c.Type == TokenVersionStr)?.Value;
			if (string.IsNullOrWhiteSpace(tokenVersion))
				return Result.Fail<(Guid UserId, int TokenVersion)>($"{RefreshStr} token \'{TokenVersionStr}\' claim was empty.");
		}
		catch (SecurityTokenException ex)
		{
			return Result.Fail<(Guid UserId, int TokenVersion)>($"{RefreshStr} token not valid. \n{ex.Message}");
		}

		if (!Guid.TryParse(userId, out var userGuid))
			return Result.Fail<(Guid UserId, int TokenVersion)>($"{nameof(userId)}  does NOT correspond guid v7 format");
		if (!int.TryParse(tokenVersion, out var version))
			return Result.Fail<(Guid UserId, int TokenVersion)>($"{TokenVersionStr} is not a valid integer.");
		return Result.Ok((userGuid, version));
	}

	public static Result<Guid> ValidateRefreshTokenAndGetId(string refreshToken)
	{
		var session = ValidateRefreshTokenAndGetSession(refreshToken);
		return session.IsFailure
			? Result.Fail<Guid>(session.Error)
			: Result.Ok(session.Value.UserId);
	}
	...
	internal static string GenerateAccessToken(User user)
	{
		...
		var claims = new List<Claim>
		{
			...
			new(TokenVersionStr, user.TokenVersion.ToString()),
		};
