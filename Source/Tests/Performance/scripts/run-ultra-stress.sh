#!/bin/bash
# Orchestrator: seed → counters → k6 ultra-stress test (1000 VUs) → report
set -euo pipefail
...
API_PORT=8090
LOG_FILE="$RESULTS_DIR/ultra_stress_full_${TIMESTAMP}.log"
mkdir -p "$RESULTS_DIR"

exec > >(tee "$LOG_FILE") 2>&1

echo "=== Ultra-Stress Test Orchestrator (1000 VUs) ==="
...
# 5. Start dotnet-counters (15 min + buffer)
echo "Starting dotnet-counters (960s)..."
bash "$SCRIPT_DIR/collect-counters.sh" "$RESULTS_DIR/counters_ultra_stress_${TIMESTAMP}.csv" 960 &
COUNTER_PID=$!

# 6. Run k6 ultra-stress test
echo "Running k6 ultra-stress test (50→1000 VUs, 15 min)..."
cd "$PERF_DIR"
ULTRA_LOG="$RESULTS_DIR/ultra_stress_${TIMESTAMP}.log"
k6 run \
  -e BASE_URL=http://localhost:${API_PORT} \
  --out json="$RESULTS_DIR/ultra_stress_${TIMESTAMP}.json" \
  --summary-export="$RESULTS_DIR/ultra_stress_summary_${TIMESTAMP}.json" \
  scenarios/ultra-stress.js 2>&1 | tee "$ULTRA_LOG" || true

# Cleanup
kill $COUNTER_PID 2>/dev/null || true
$DC exec -T perf-api sh -c 'kill $(pidof dotnet-counters) 2>/dev/null; sleep 2' 2>/dev/null || true
$DC cp perf-api:/tmp/counters.csv "$RESULTS_DIR/counters_ultra_stress_${TIMESTAMP}.csv" 2>/dev/null || true

echo ""
echo "=== Ultra-stress test complete ==="
echo "Report: $RESULTS_DIR/ultra_stress_report.html"
echo "Counters: $RESULTS_DIR/counters_ultra_stress_${TIMESTAMP}.csv"
