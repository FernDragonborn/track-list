#!/bin/bash
# Orchestrator: seed → counters → k6 load test → report
set -euo pipefail
...
API_PORT=8090
LOG_FILE="$RESULTS_DIR/load_full_${TIMESTAMP}.log"
mkdir -p "$RESULTS_DIR"

exec > >(tee "$LOG_FILE") 2>&1

echo "=== Load Test Orchestrator ==="
...
# 4. Seed (skip if seed-data.json exists)
...
# 5. Start dotnet-counters (5 min + buffer)
echo "Starting dotnet-counters (360s)..."
bash "$SCRIPT_DIR/collect-counters.sh" "$RESULTS_DIR/counters_load_${TIMESTAMP}.csv" 360 &
COUNTER_PID=$!

# 6. Run k6 load test
echo "Running k6 load test (50 VUs, 5 min)..."
cd "$PERF_DIR"
LOAD_LOG="$RESULTS_DIR/load_${TIMESTAMP}.log"
k6 run \
  ...
  scenarios/load.js 2>&1 | tee "$LOAD_LOG" || true

# Cleanup
kill $COUNTER_PID 2>/dev/null || true
$DC exec -T perf-api sh -c 'kill $(pidof dotnet-counters) 2>/dev/null; sleep 2' 2>/dev/null || true
$DC cp perf-api:/tmp/counters.csv "$RESULTS_DIR/counters_load_${TIMESTAMP}.csv" 2>/dev/null || true

echo ""
echo "=== Load test complete ==="
echo "Report: $RESULTS_DIR/load_report.html"
echo "Counters: $RESULTS_DIR/counters_load_${TIMESTAMP}.csv"
